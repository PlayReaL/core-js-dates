...
function getNextFriday(date) {
  ...
  nextFriday.setDate(date.getDate() + ((5 + 7 - 1 - date.getDay()) % 7) + 1);
  return nextFriday;
}
...
function getWeekNumberByDate(date) {
  ...
  const firstWeekDay = firstDay - ((firstDay.getDay() - 1 + 7) % 7) * dailyMs;
  const diff = date - firstWeekDay + dailyMs;
  return Math.ceil(diff / (7 * dailyMs));
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const cycle = countWorkDays + countOffDays;
  for (let i = 0; i < diffDays; i += 1) {
    if (i % cycle < countWorkDays) {
